...
describe('LlmService.portfolioSummary', () => {
  it('returns an unconfigured result without touching the portfolio or the model, when no key is set', async () => {
    ...
    const service = new LlmService(client, portfolio, performance);
  ...
  it('builds facts from the portfolio and performance services and passes them to the client', async () => {
  ...
  it('returns a structured, non-throwing error when the model call fails', async () => {
  ...
  it('exposes isConfigured by delegating to the client', () => {
    const client: LlmClient = { isConfigured: () => true, complete: vi.fn() };
